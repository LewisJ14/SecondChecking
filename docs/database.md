...
schema. Orders are stored once in the `orders` table, while assigned device
...
[`sql/001_create_orders.sql`](../sql/001_create_orders.sql).

## `orders`

The `orders` table captures the marketplace metadata for each line item so the
...
| `order_id` | `BIGINT` | Foreign key pointing at `orders.id`. |
...
current application logic. The foreign key inherits cascade rules from `orders`
so removing an order automatically clears related serial assignments.

## Applying the schema

Run the statements in [`sql/001_create_orders.sql`](../sql/001_create_orders.sql)
against your MySQL instance to migrate to the consolidated schema:

```sql
SOURCE sql/001_create_orders.sql;
```
